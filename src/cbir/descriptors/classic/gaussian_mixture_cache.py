...
`rootsift_cache.py`). Unlike `Vocabulary` (shared by BoW/VLAD, see `vocabulary_cache.py`),
...
`vocabulary_cache.py`'s docstring for why that distinction matters.
...
def cached_train(dataset: str, descriptors: np.ndarray, k: int, seed: int) -> GaussianMixture:
