...
from cbir.descriptors.classic.vlad import encode, fit_and_encode, normalize, raw_vlad
...
def test_raw_vlad_sums_residuals_per_word():
    ...
    assert list(raw_vlad(vocab, IMAGE)) == list(RAW)


def test_raw_vlad_empty_image_is_zero():
    vocab = Vocabulary(CENTERS)
    assert list(raw_vlad(vocab, np.zeros((0, 2), dtype=np.float32))) == [0.0, 0.0, 0.0, 0.0]


def test_normalize_global_l2_only():
    out = normalize(RAW[None, :], k=2, d=2, intra_norm=False)
...
def test_normalize_intra_then_global():
    out = normalize(RAW[None, :], k=2, d=2, intra_norm=True)
...
def test_normalize_power_law_then_global():
    out = normalize(RAW[None, :], k=2, d=2, intra_norm=False, power=0.5)
...
def test_normalize_zero_row_stays_zero():
    out = normalize(np.zeros((1, 4), dtype=np.float32), k=2, d=2, intra_norm=True, power=0.5)
...
def test_encode_produces_unit_rows():
    vocab = Vocabulary(CENTERS)
    vectors = encode(vocab, [IMAGE, IMAGE])
...
def test_fit_and_encode_trains_vocabulary_and_encodes_database_and_queries(tmp_cache):
    ...
    vocabulary, db_vectors, q_vectors = fit_and_encode(inputs, k=2, seed=0)

    assert vocabulary.k == 2
