...
import cbir.descriptors.classic.cache.rootsift as cache_module
from cbir.descriptors.classic.cache.rootsift import RootSIFTCache


def _fake_rootsift(monkeypatch, extract_many):
    """Install a stand-in RootSIFT whose extract_many is `extract_many`.

    The cache builds its own extractor (`RootSIFT().extract_many(images)`), so the
    seam for tests is the class the cache module looks up, not a module-level function.
    """

    class FakeRootSIFT:
        def extract_many(self, images):
            return extract_many(images)

    monkeypatch.setattr(cache_module, "RootSIFT", FakeRootSIFT)
...
def test_pack_unpack_round_trip():
    ...
    flat, offsets = RootSIFTCache.pack(descriptors)
    assert flat.shape == (3, 2)
    assert offsets.tolist() == [0, 2, 2, 3]

    restored = RootSIFTCache.unpack(flat, offsets)
...
def test_cached_extract_many_writes_and_reuses_cache(tmp_cache, monkeypatch):
    ...
    _fake_rootsift(monkeypatch, fake_extract_many)

    images = _images(3)
    first = RootSIFTCache.extract_many("roxford5k", "database", images)
    assert len(calls) == 1
    assert (tmp_cache / "rootsift" / "roxford5k_database.npz").exists()

    second = RootSIFTCache.extract_many("roxford5k", "database", images)
...
def test_cached_extract_many_never_consumes_images_on_cache_hit(tmp_cache, monkeypatch):
    ...
    # untouched on a hit.
    _fake_rootsift(monkeypatch, lambda images: [np.zeros((1, 128), dtype=np.float32)])

    RootSIFTCache.extract_many("roxford5k", "database", _images(1))  # populates the cache

    result = RootSIFTCache.extract_many("roxford5k", "database", _ExplodingImages())
    assert len(result) == 1
...
def test_cached_extract_many_keys_are_independent_per_dataset_and_role(tmp_cache, monkeypatch):
    ...
    RootSIFTCache.extract_many("roxford5k", "database", _images(1))
    RootSIFTCache.extract_many("roxford5k", "query", _images(1))
    RootSIFTCache.extract_many("rparis6k", "database", _images(1))
...
def test_cached_pooled_descriptors_returns_flat_array_directly(tmp_cache, monkeypatch):
    ...
    pooled = RootSIFTCache.pooled("roxford5k", "database", _images(2))
    assert pooled.shape == (4, 128)

    # Second call must be served from disk, not by recomputing via extract_many.
    pooled_again = RootSIFTCache.pooled("roxford5k", "database", _ExplodingImages())
...
def test_cached_extract_many_and_cached_pooled_descriptors_share_one_cache_entry(tmp_cache, monkeypatch):
    ...
    per_image = RootSIFTCache.extract_many("roxford5k", "database", _images(2))
    pooled = RootSIFTCache.pooled("roxford5k", "database", _ExplodingImages())
