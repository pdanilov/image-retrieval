...
from cbir.descriptors.classic.fisher import encode, fisher_vector, fit_and_encode, normalize
...
def test_fisher_vector_single_component_hand_computed():
    ...
    fv = fisher_vector(gmm, np.array([[2.0]], dtype=np.float32))
...
def test_fisher_vector_layout_is_mu_then_sigma():
    ...
    fv = fisher_vector(gmm, np.array([[0.0]], dtype=np.float32))
...
def test_fisher_vector_dimensionality_is_two_k_d():
    ...
    fv = fisher_vector(gmm, np.random.default_rng(0).normal(size=(7, 4)))
...
def test_fisher_vector_empty_image_is_zero():
    ...
    assert list(fisher_vector(gmm, np.zeros((0, 1), dtype=np.float32))) == [0.0, 0.0]


def test_normalize_power_law_then_global_l2():
    raw = np.array([[2.0, 3.0 / np.sqrt(2.0)]], dtype=np.float32)
    out = normalize(raw, power=0.5)
...
def test_normalize_zero_row_stays_zero():
    out = normalize(np.zeros((1, 4), dtype=np.float32), power=0.5)
...
def test_encode_produces_unit_rows_of_expected_width():
    ...
    vectors = encode(gmm, images)
    assert vectors.shape == (2, 2 * 2 * 1)
    assert np.linalg.norm(vectors[0]) == pytest.approx(1.0)


def test_fit_and_encode_trains_gmm_and_encodes_database_and_queries(tmp_cache):
    ...
    model, db_vectors, q_vectors = fit_and_encode(inputs, k=2, seed=0)
